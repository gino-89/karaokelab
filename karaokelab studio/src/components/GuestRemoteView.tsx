...
export const GuestRemoteView: React.FC = () => {
  ...
  useEffect(() => {
    // Load songs and profiles from local database
    getSongsFromDB().then((songs) => setSavedSongs(songs));
    setYoutubeFavorites(getYouTubeFavoritesFromStorage());
    setProfiles(getProfilesFromStorage());

    // Listen to live TV status
    const unsub = tvBroadcast.onStateUpdate((state) => {
      setTvState(state);
    });
    return () => unsub();
  }, []);
  ...
  const handleRequestSong = (title: string, artist?: string, isYouTube = false, videoId?: string) => {
          ...
          <div className="flex flex-col gap-1.5 mt-2">
            ...
            {filteredLocalSongs.map((song) => (
              ...
              >
                ...
                <button
                  onClick={() => handleRequestSong(song.title, song.artist, false)}
