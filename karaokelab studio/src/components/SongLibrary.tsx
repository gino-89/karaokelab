...
import { importSongsFromFolder } from '../services/folderSyncService';
...
export const SongLibrary: React.FC<SongLibraryProps> = React.memo(({
  ...
  const handleExportFullZip = async () => {
    ...
    try {
      ...
      setBackupFeedback(`Error al exportar: ${err?.message || 'Fallo de compresión'}`);
    } finally {
      setIsExportingAudioZip(false);
      setTimeout(() => setBackupFeedback(null), 5000);
    }
  ...
  return (
    <div className="flex flex-col gap-3 relative">
      {/* ── Upload Hero Box (Compact) ──────────────────────────────── */}
      <div
        onDragOver={(e) => { e.preventDefault(); setDragOver(true); }}
        onDragLeave={() => setDragOver(false)}
        onDrop={handleDrop}
        onClick={() => fileInputRef.current?.click()}
        className={`relative cursor-pointer rounded-2xl border-2 border-dashed transition-all duration-300 overflow-hidden shrink-0 ${dragOver
          ? 'border-[#00f0ff] bg-[#00f0ff]/15 shadow-[0_0_30px_rgba(0,240,255,0.4)]'
          : hasActive
            ? 'border-[#00f0ff]/50 bg-[#00f0ff]/5 animate-pulse'
            : 'border-slate-700/80 bg-slate-900/60 hover:border-[#00f0ff]/50 hover:bg-[#00f0ff]/5'
          }`}
      >
        <input
          ref={fileInputRef}
        ...
        <div className="flex flex-col items-center gap-1.5 py-3 px-4 text-center">
          {isProcessingUpload || hasActive ? (
            <>
              <Loader2 className="w-5 h-5 text-[#00f0ff] animate-spin shrink-0" />

              {/* Batch Counter (e.g., Canción 2 de 5) */}
              <div className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-[#00f0ff]">
                <span>
                  {uploadTotalCount > 1
                    ? `Canción ${uploadCurrentIndex} de ${uploadTotalCount}`
                    : activeProcessing.length > 1
                      ? `Canción 1 de ${activeProcessing.length}`
                      : 'Procesando Canción'}
                </span>
                <span className="font-mono text-cyan-300">({uploadProgress || activeProcessing[0]?.progress || 0}%)</span>
              </div>

              {/* Exact Track Name */}
              <span className="text-[11px] font-bold text-white truncate max-w-[220px]">
                🎵 {uploadFileName || activeProcessing[0]?.fileName || 'Analizando audio...'}
              </span>

              {/* Progress Bar */}
              <div className="w-48 h-1.5 bg-slate-950 rounded-full overflow-hidden border border-slate-800 my-0.5 shadow-inner">
                <div
                  className="h-full bg-gradient-to-r from-[#00f0ff] via-[#bd00ff] to-[#ff007f] rounded-full transition-all duration-200 shadow-[0_0_8px_#00f0ff]"
                  style={{ width: `${uploadProgress || activeProcessing[0]?.progress || 0}%` }}
                />
              </div>

              {/* Step Info */}
              <span className="text-[10px] font-mono text-[#00ff9d] truncate max-w-[220px]">
                {uploadStep || activeProcessing[0]?.currentStep || 'Separando voz con STFT...'}
              </span>
            </>
          ) : (
            <>
              <UploadCloud className={`w-6 h-6 transition-colors ${dragOver ? 'text-[#00f0ff]' : 'text-slate-400'}`} />
              ...
              {onOpenYouTubeModal && (
                <button
                  ...
                  className="mt-1 px-3 py-1 rounded-xl bg-red-600/25 hover:bg-red-600/40 border border-red-500/50 text-red-300 text-xs font-bold transition-all shadow-md cursor-pointer flex items-center gap-1.5 hover:scale-105 active:scale-95"
                  title="Buscar en YouTube e importar pista a la biblioteca"
                >
      ...
      {/* ── Library Card (Fixed Stable Height) ─────────────────────────────── */}
      <div className="bg-[#0c0e17] border border-slate-700/70 rounded-2xl flex flex-col shadow-lg h-[800px] relative">
        ...
        <input
          ...
          type="file"
          accept=".zip,.karaokelab,.json,application/json,application/zip,application/x-zip-compressed"
        ...
        {/* Songs List */}
        <div className="flex flex-col flex-1 overflow-y-auto divide-y divide-slate-850 h-full rounded-b-2xl">
          ...
          <div className="w-full max-w-lg bg-[#0c0e17] border border-slate-700/80 rounded-2xl p-6 flex flex-col gap-5 shadow-2xl animate-in fade-in zoom-in-95 duration-150">
            ...
            {!isExportingAudioZip && (
              <div className="flex flex-col gap-2.5">
                {/* Option 1: INCREMENTAL PACKAGE (Super Fast for new/modified songs) */}
                <div
                  onClick={handleExportIncrementalZip}
                  className="p-3.5 rounded-xl border border-amber-500/40 bg-amber-950/20 hover:bg-amber-900/30 cursor-pointer transition-all flex flex-col gap-1.5 hover:scale-[1.01] active:scale-[0.99] group shadow-sm hover:shadow-[0_0_15px_rgba(245,158,11,0.2)]"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-base">⚡</span>
                      <span className="text-xs font-bold text-amber-300 uppercase tracking-wider">
                        Exportar Solo Nuevas / Modificadas ({modifiedOrNewSongs.length} temas)
                      </span>
                    </div>
                    <span className="px-2 py-0.5 rounded-full text-[9px] font-mono font-black bg-amber-500/20 text-amber-300 border border-amber-500/40">
                      ⚡ SÚPER RÁPIDO
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-300 leading-relaxed">
                    Genera un paquete único <strong>con solo las canciones agregadas o editadas</strong> para transferir al Player en segundos.
                  </p>
                </div>

                {/* Option 2: UPDATE AN EXISTING PACKAGE FILE (1-2 seconds) */}
                <div
                  onClick={() => updateExistingZipFileInputRef.current?.click()}
                  className="p-3.5 rounded-xl border border-indigo-500/40 bg-indigo-950/20 hover:bg-indigo-900/30 cursor-pointer transition-all flex flex-col gap-1.5 hover:scale-[1.01] active:scale-[0.99] group shadow-sm hover:shadow-[0_0_15px_rgba(99,102,241,0.2)]"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-base">🔄</span>
                      <span className="text-xs font-bold text-indigo-300 uppercase tracking-wider">
                        Inyectar en Paquete (.klab / .zip) Existente
                      </span>
                    </div>
                    <span className="px-2 py-0.5 rounded-full text-[9px] font-mono font-black bg-indigo-500/20 text-indigo-300 border border-indigo-500/40">
                      MISMO ARCHIVO
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-300 leading-relaxed">
                    Selecciona tu archivo de biblioteca existente y le agrega las canciones nuevas en <strong>1 segundo</strong> manteniendo el mismo archivo sin duplicados.
                  </p>
                </div>
                <input
                  ref={updateExistingZipFileInputRef}
                  type="file"
                  accept=".zip,.klab,application/zip,application/x-zip-compressed"
                  onChange={handleUpdateExistingZipFile}
                  className="hidden"
                />

                {/* Option 3: FULL AUDIO PACKAGE (All songs) */}
                <div
                  onClick={handleExportFullZip}
                  className="p-3.5 rounded-xl border border-emerald-500/40 bg-emerald-950/20 hover:bg-emerald-900/30 cursor-pointer transition-all flex flex-col gap-1.5 hover:scale-[1.01] active:scale-[0.99] group shadow-sm"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-base">📦</span>
                      <span className="text-xs font-bold text-emerald-300 uppercase tracking-wider">
                        Exportar Archivo Único Auto-Contenido ({savedSongs.length} temas)
                      </span>
                    </div>
                  ...
                  <p className="text-[11px] text-slate-300 leading-relaxed">
                    Empaqueta todas las pistas de audio (instrumental + voz aislada), letras sincronizadas (.LRC), división de cantantes (Hombre/Mujer/Dúo) y tonos en un único archivo.
                  </p>
                </div>

                {/* Option 4: METADATA ONLY JSON (Fast & Lightweight) */}
                <div
                  onClick={handleExportMetadataJson}
                  className="p-3.5 rounded-xl border border-slate-700 bg-slate-900/60 hover:bg-slate-800/80 cursor-pointer transition-all flex flex-col gap-1.5 hover:scale-[1.01] active:scale-[0.99] group"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-base">📄</span>
                      <span className="text-xs font-bold text-[#00f0ff] uppercase tracking-wider">
                        Solo Letras y Ajustes (.JSON)
                      </span>
                    </div>
                  ...
                  <p className="text-[11px] text-slate-400 leading-relaxed">
                    Descarga instantánea de respaldo solo con las letras, marcas de tiempo milimétricas, división de voces y perfiles (sin los archivos de sonido pesados).
                  </p>
                </div>
              </div>
            )}
