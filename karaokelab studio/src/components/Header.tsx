          ...
          <div className="relative" ref={menuContainerRef}>
            <button
              ...
              className={`px-2.5 py-1 rounded-xl border text-[11px] font-bold uppercase transition-all cursor-pointer flex items-center gap-1.5 ${
                showImportMenu
                  ? 'border-[#00f0ff] bg-[#00f0ff]/20 text-[#00f0ff] shadow-[0_0_12px_rgba(0,240,255,0.4)]'
                  : 'border-slate-700 bg-slate-850 text-slate-200 hover:bg-slate-800 hover:border-cyan-400/50'
              }`}
              title="Importar canción suelta o carpeta completa"
            >
            ...
            {showImportMenu && (
              <div className="absolute right-0 mt-1.5 w-64 rounded-xl bg-slate-900/98 border border-slate-700 shadow-2xl p-1.5 z-50 animate-in fade-in zoom-in-95 flex flex-col gap-1 backdrop-blur-md">
                <button
                  onClick={() => {
                    setShowImportMenu(false);
                    fileInputRef.current?.click();
                  }}
                  className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-left text-xs font-bold text-slate-200 hover:text-white hover:bg-slate-800 cursor-pointer transition-colors"
                >
                  <FileAudio className="w-4 h-4 text-[#00f0ff] shrink-0" />
                  <div>
                    <span className="block">Subir Canción (Archivo)</span>
                    <span className="block text-[10px] text-slate-400 font-normal">MP3, WAV, FLAC, M4A</span>
                  </div>
                </button>
                ...
                {onImportSyncedFolder && (
                  <button
                    ...
                    className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-left text-xs font-bold text-amber-300 hover:text-amber-200 hover:bg-amber-500/15 cursor-pointer transition-colors border-t border-slate-800/80"
                  >
                    <FolderUp className="w-4 h-4 text-amber-400 shrink-0" />
                    <div>
                      <span className="block font-black">📁 Cargar Carpeta / USB Sincronizada</span>
                      <span className="block text-[10px] text-amber-400/80 font-normal">Carga todo el repertorio en 1s</span>
                    </div>
                  </button>
                )}
                ...
                <button
                  ...
                  className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-left text-xs font-bold text-slate-300 hover:text-white hover:bg-slate-800 cursor-pointer transition-colors border-t border-slate-800/80"
                >
                  <Upload className="w-4 h-4 text-cyan-400 shrink-0" />
                  <div>
                    <span className="block">Lote de Archivos Sueltos</span>
                    <span className="block text-[10px] text-slate-400 font-normal">Procesar archivos sin sincronizar</span>
                  </div>
                </button>
              </div>
            )}
          ...
          {onSyncToFolder && (
            <div className="flex items-center">
              ...
              >
                ...
                <span>{isFolderSyncing ? 'Sincronizando...' : 'Sincronizar Player'}</span>
              </button>
