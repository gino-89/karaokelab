          ...
          <div className="flex items-center gap-2.5 min-w-0 flex-1">
            ...
            <div className="flex flex-col min-w-0 flex-1">
              ...
              <div className="queue-song-meta flex items-center gap-1.5 text-[10px] xl:text-[11.5px] font-mono text-slate-300 truncate mt-0.5">
                ...
                <span className="hidden xl:inline text-amber-300 font-bold">{currentSong.key || 'Am'}</span>
              </div>
            </div>
          </div>
              ...
              >
                ...
                <div className="flex flex-col min-w-0 flex-1 gap-0.5">
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {item.requestedBy && (
                      <span
                        className="inline-flex items-center gap-1 text-[9px] xl:text-[9.5px] font-bold text-cyan-300 bg-cyan-950/80 px-1.5 py-0.5 rounded-full border border-cyan-500/40 shrink-0 shadow-sm"
                        title={`Canción pedida por ${item.requestedBy}${item.tableNumber ? ` (${item.tableNumber})` : ''}`}
                      >
                        <span className="text-[10px]">🎤</span>
                        <span>{item.requestedBy}</span>
                        {item.tableNumber && (
                          <span className="px-1.5 py-0.2 rounded bg-pink-500/30 text-pink-300 border border-pink-500/50 text-[8.5px] xl:text-[9px] font-black ml-0.5">
                            🪑 {item.tableNumber}
                          </span>
                        )}
                      </span>
                  ...
                  {isReady && (
                    <div className="queue-song-meta flex items-center gap-1.5 text-[10px] xl:text-[11.5px] font-mono text-slate-400 truncate mt-0.5">
                      ...
                      {(item.songData?.key || item.songData?.bpm) ? (
                        <>
                          <span className="hidden xl:inline">·</span>
                          <span className="hidden xl:inline text-amber-300 font-bold">{item.songData?.key || 'Am'}</span>
                        </>
                      ) : null}
                    </div>
                  )}
                  ...
                  {isProcessing && item.currentStep && (
                    <span className="text-[9px] xl:text-[10px] font-mono text-[#00f0ff] truncate">{item.currentStep}</span>
                  )}
                  {isError && (
                    <span className="text-[9px] xl:text-[10px] font-mono text-[#ff007f]">{item.errorMsg || 'Error al procesar'}</span>
                  )}
                  {isProcessing && (
